...
full_sample <- "healthy_bcells_all"
...
full_data <- readRDS(file.path(base_dir, "results", full_sample, 
                               "R_analysis", "rda_obj", "seurat_processed.rds"))


tcells <- subset(x = full_data, subset = RNA_celltype == "CD8pos_T")

saveRDS(tcells, file.path(save_dir, "rda_obj", "seurat_tcells.rds"))

# IgM low using t cells
ADT_data <- GetAssayData(tcells, assay = "ADT", slot = "data") %>%
  t %>%
  data.frame
...
igm_cutoff <- quantile(ADT_data$IgM, prob = 0.95)
...
cd21_high_cutoff <- quantile(ADT_data$CD21, prob = 0.25)
cd21_low_cutoff <- quantile(ADT_data$CD21, prob = 0.1)
...
CD21_plot <- ADT_data_long %>%
  ...
  ggplot2::geom_density() +
  ggplot2::scale_fill_manual(values = protein_colors) + 
  ggplot2::geom_vline(xintercept = cd21_high_cutoff) +
  ggplot2::geom_vline(xintercept = cd21_low_cutoff)
...
ADT_data$CD21_high_cutoff <- ADT_data$CD21 > cd21_high_cutoff
ADT_data$CD21_low_cutoff <- ADT_data$CD21 < cd21_low_cutoff
ADT_data$CD27_cutoff <- ADT_data$CD27.1 > cd27_cutoff

ADT_data$pass_filters <- ADT_data$IgM_cutoff + 
  ADT_data$IgD_cutoff + 
  ADT_data$CD21_low_cutoff

new_metadata <- ADT_data %>%
  dplyr::select(c("IgM_cutoff", "IgD_cutoff", "CD21_low_cutoff",
                  "CD21_high_cutoff", "CD27_cutoff",
                  "pass_filters"))
...
plotDimRed(seurat_data, "CD21_low_cutoff", plot_type = "rna.umap")
plotDimRed(seurat_data, "CD21_high_cutoff", plot_type = "rna.umap")
