...
start_sample <- "healthy_bcells_all"
...
# Set directories
sample_dir <- file.path(here("results"), sample)
save_dir <- file.path(here("results"), sample, "R_analysis")
read_dir <- file.path(here("results"), start_sample, "R_analysis")

# Make directories
ifelse(!dir.exists(sample_dir), dir.create(sample_dir), FALSE)
...
# Read in the data
seurat_data <- readRDS(file.path(read_dir, "rda_obj", "seurat_processed.rds"))

  
# Subset to only b cells
seurat_data <- subset(seurat_data, subset = RNA_celltype %in% c("Mem_B",
                                                                "Bulk_B"))
